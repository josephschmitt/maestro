...
pub async fn resume_agent(
    ...
    let (card_id, session_id, agent_type) = db.with_conn(|conn| {
        conn.query_row(
            "SELECT card_id, session_id, agent_type FROM agent_workspaces WHERE id = ?1",
            rusqlite::params![workspace_id],
    ...
    let (card_title, card_description, parent_title, parent_description, project_agent_config, status_group) =
        db.with_conn(|conn| {
            let (title, description, parent_id, status_id): (String, String, Option<String>, String) = conn
                .query_row(
                    "SELECT c.title, c.description, c.parent_id, c.status_id FROM cards c WHERE c.id = ?1 AND c.project_id = ?2",
                    rusqlite::params![card_id, project_id],
            ...
            let group: String = conn
                .query_row(
                    "SELECT \"group\" FROM statuses WHERE id = ?1",
            ...
            Ok((title, description, parent_title, parent_description, project_agent_config, group))
        })?;
    ...
    let workspace = db.with_conn(|conn| {
        conn.execute(
            "INSERT INTO agent_workspaces (id, card_id, agent_type, status, session_id, pid, attached_at) \
             VALUES (?1, ?2, ?3, 'running', ?4, ?5, ?6)",
            rusqlite::params![new_workspace_id, card_id, agent_type, session_id, pid as i64, now],
        )
        .map_err(|e| format!("Failed to create workspace: {e}"))?;
    ...
    for entry in entries.flatten() {
        ...
        if let Ok(db) = open_project_db(&base_path, &project_id) {
            let _ = db.with_conn(|conn| {
                let mut stmt = conn
                    .prepare(&format!("{WORKSPACE_SELECT} WHERE status = 'running'"))
                    .map_err(|e| format!("{e}"))?;
