...
function defaultStatusPromptsForGroup(group: string): string[] {
	switch (group) {
		case 'Backlog':
...
export function create_status(args: Record<string, unknown>): Status {
	...
	const status: Status = {
		...
		status_prompts:
			(args.statusPrompts as string[]) ??
			defaultStatusPromptsForGroup(args.group as string),
		created_at: nowISO()
	};
...
export function update_status(args: Record<string, unknown>): Status {
	...
	if (args.statusPrompts !== undefined) status.status_prompts = args.statusPrompts as string[];
	return status;
}
